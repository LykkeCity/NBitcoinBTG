...
using NBitcoinBTG.BouncyCastle.Crypto.Signers;
...
namespace NBitcoinBTG.Crypto
{
	internal class ECKey
	{
		...
		static ECKey()
		{
			_Secp256k1 = NBitcoinBTG.BouncyCastle.Crypto.EC.CustomNamedCurves.Secp256k1;
		...
		{
			if(isPrivate)
				_Key = new ECPrivateKeyParameters(new NBitcoinBTG.BouncyCastle.Math.BigInteger(1, vch), DomainParameter);
			else
			{
		...
		public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, uint256 message, bool compressed)
		{
			...
			var n = curve.N;
			var i = NBitcoinBTG.BouncyCastle.Math.BigInteger.ValueOf((long)recId / 2);
			...
			var e = new NBitcoinBTG.BouncyCastle.Math.BigInteger(1, message.ToBytes());
			...
			var eInv = NBitcoinBTG.BouncyCastle.Math.BigInteger.Zero.Subtract(e).Mod(n);
		...
		private static ECPoint DecompressKey(NBitcoinBTG.BouncyCastle.Math.BigInteger xBN, bool yBit)
		{
